          ...
          _list.forEach(element => {
            ...
            if (_child) {
              _child.forEach(element => {
                ...
                //lấy ra cấp 3
                let _three = taxonomys.filter(function(e, i) {
                return ((e.parent_id == element.id) && e.taxonomy == _value);
                });
                
                if (_three) {
                  _three.forEach(element => {
                    _content += '<option value="' + element.id + '">- - - -' + element.title + ' </option>';

                    //lấy ra cấp 4
                    let _four = taxonomys.filter(function(e, i) {
                    return ((e.parent_id == element.id) && e.taxonomy == _value);
                    });
                    
                    if (_four) {
                      _four.forEach(element => {
                        _content += '<option value="' + element.id + '">- - - - - -' + element.title + ' </option>';

                        //Lấy ra cấp 5
                        let _five = taxonomys.filter(function(e, i) {
                        return ((e.parent_id == element.id) && e.taxonomy == _value);
                        });
                        
                        if (_five) {
                          _five.forEach(element => {
                            _content += '<option value="' + element.id + '">- - - - - - - -' + element.title + ' </option>';
                          });
                        }
